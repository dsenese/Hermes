...
class ContextAwareTextFormatter: ObservableObject {
    ...
    func formatText(_ text: String, context: ContextInfo, existingContent: TextFieldContent) -> FormattedTextResult {
        ...
        // Enforce dictionary capitalization/corrections exactly as saved
        let corrected = dictionaryManager.applyCorrections(to: result.formattedText)
        if corrected != result.formattedText {
            print("📖 Dictionary enforced after formatting")
        }

        print("📝 Formatted text: '\(text.prefix(30))...' → '\(corrected.prefix(30))...' (\(formatter.name))")

        return FormattedTextResult(
            formattedText: corrected,
            originalText: result.originalText,
            appliedFormatting: result.appliedFormatting,
