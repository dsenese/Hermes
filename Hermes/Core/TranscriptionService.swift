...
class TranscriptionService: ObservableObject {
    ...
    /// Latency is measured from end of recording to completion of all processing (transcription + formatting)
    func transcribeCompleteSession(appBundleId: String? = nil) async -> String {
        ...
        isTranscribing = true
        // Measure latency from here (recording has already stopped when this is called)
        let latencyStart = Date()
        let (rawTranscription, confidence) = await transcribeAudioDirectly(floatArray: audioBuffer, suppressPublish: true)
        ...
        // Compute end-to-end latency (stop -> final text ready)
        let endToEndLatency = Date().timeIntervalSince(latencyStart)

        // Publish a final result with end-to-end latency for UI consumers (e.g., Notes)
        if !finalResult.isEmpty {
            let published = HermesTranscriptionResult(
                text: finalResult,
                type: .final,
            ...
            transcriptionSubject.send(published)
        }
    ...
    private func transcribeAudioDirectly(floatArray: [Float], suppressPublish: Bool = false) async -> (text: String, confidence: Double) {
        ...
        do {
            ...
            let confidence = calculateConfidence(from: results.first)

            // Publish only if not suppressed (streaming scenarios)
            if !suppressPublish {
            ...
            return (transcriptionText, confidence)
